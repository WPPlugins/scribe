...
//		scribe_seo.custom_tb = true;
	...
	$('#scribe-keyword-research-twitter-headlines-term').change(function(event) {
		if(window.twitter_widget) {
			window.twitter_widget.stop().setSearch($(this).val() + ' -RT lang:en').render().start();
		}
	}).change();
	...
	$('#scribe-keyword-research-google-plus-headlines-term').change(function(event) {
		...
		scribe_seo.research_google_plus(term_value, function(data, status) {
...
//			top.scribe_seo.setup();
			...
			$('#scribe-build-links-ajax-feedback').css('visibility', 'hidden');
//			$('#scribe-link-building-internal-links-tab-identifier').click();
...
var scribe_seo = function() {
	...
	object.setup_keyword_research = function() {
		...
		if('' == target_term) {
			...
			$keyword_research_input_container.css('border-bottom-style', 'solid');
	...
	object.research_google_plus = function(keyword, callback) {
		if('' == keyword) {
			alert('Please enter a keyword to search for.');
			return;
		}
		...
		$.post(
			ajaxurl,
			{
				'action': 'scribe_research_google_plus',
				'scribe-google-plus-nonce': $('#scribe-google-plus-nonce').val(),
				'scribe-keyword': keyword
			},
			function(data, status) {
				if(data.error) {
	...
	object.set_twitter_screen_name = function(username) {
		var $twitter_connect = $('#scribe-link-building-meta-box-twitter-connect');
		$twitter_connect.attr('data-screen-name', username);
	};
