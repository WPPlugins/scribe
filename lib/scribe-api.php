...
/*
			if(self::DEBUG) {
				...
				self::$api_base = self::$test_base;
				self::$api_base_secure = self::$test_base;
			}
*/
		}
...
update_option( 'aa', $get_data );
			$request_url = add_query_arg( $get_data, $request_url );
			
			if($is_post || !empty($post_data)) {
				$body = self::urlencode_json_encode($post_data);

				$response = wp_remote_post($request_url, array('timeout' => 60, 'sslverify' => false, 'body' => $body, 'headers' => array('Content-Type' => 'application/json')));
			} else {
		...
		private static function urlencode_json_encode($item) {
			return json_encode(self::recursive_urlencode($item));
		}
		
		private static function urldecode_json_decode($string) {
