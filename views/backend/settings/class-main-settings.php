...
class Scribe_Main_Settings extends Scribe_Admin_Boxes {
	...
	function __construct() {
		...
		add_action( 'scribe_admin_before_metaboxes', array( $this, 'premise_notice' ) );
	}
...
?>
	 ...
	 * notice for Premise
	 */
	function premise_notice( $page_hook ) {

		if ( $page_hook != $this->pagehook || get_option( Scribe_SEO::SHOW_PREMISE_NAG_KEY ) )
			return;

		?>
		<input type="hidden" name="<?php echo $this->get_field_name( 'version' ); ?>" value="<?php echo Scribe_SEO::VERSION; ?>" />
		<?php

		if ( isset( $_GET['scribe-dismiss-premise-nag'] ) && $_GET['scribe-dismiss-premise-nag'] == '1' ) {

			update_option( Scribe_SEO::SHOW_PREMISE_NAG_KEY, '1' );
			return;

		}

		$dismiss_nag_url = add_query_arg( array( 'scribe-dismiss-premise-nag' => 1 ), menu_page_url( Scribe_SEO::SETTINGS_TOP_PAGE_SLUG ) );
		?>
			<div id="scribe-premise-nav" class="updated settings-error">
				<p>
					<?php esc_html_e( 'Have you tried our landing page software? Premise is the perfect companion to Scribe, helping you creating optimized landing pages directly in WordPress.', 'scribeseo' ); ?> <a target="_blank" href="http://getpremise.com"><?php esc_html_e( 'Click here to learn more.', 'scribeseo' ); ?></a><br />
				</p>
				<p class="alignright"><a href="<?php echo esc_url( $dismiss_nag_url ); ?>"><?php esc_html_e( 'Dismiss', 'scribeseo' ); ?></a></p>
				<div class="clear"></div>
			</div>
		<?php
