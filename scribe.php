...
/*
 ...
 Description: Scribe for WordPress has been discontinued, and the Scribe API has been deactivated. Please feel free to delete this plugin.
 Version: 4.0.10
...
if(!class_exists('Scribe_SEO')) {
	class Scribe_SEO {
		...
		//// URLS
		const SEO_NEWS_FEED_URL = 'http://feeds.feedburner.com/SearchEngineJournal';
		...
		private static function add_actions() {
			...
			add_action( 'admin_init', array( __CLASS__, 'handle_versions' ) );
			...
			add_action( 'wp_ajax_scribe_research_google_plus', array( __CLASS__, 'ajax_handle_research_google_plus' ) );
		...
		private static function initialize_scribe_api() {
			if(null === self::$scribe_api) {
				...
				self::$scribe_api = new Scribe_API($settings['api-key'], 1 == $settings['security-method']);
			}
		}
		...
		public static function ajax_handle_research_keyword() {
			...
			if(isset($data['scribe-research-keyword-nonce']) && wp_verify_nonce($data['scribe-research-keyword-nonce'], 'scribe-research-keyword')) {
				...
				$previous_suggestions = self::get_archived_keyword_suggestions( $post_id, $user_id );
		...
		public static function ajax_handle_research_google_plus() {

			$data = wp_parse_args( stripslashes_deep( $_REQUEST ), array(
				'scribe-google-plus-nonce' => '',
				'scribe-keyword' => '',
			) );

			$results = '<p>' . __( 'Invalid request.', 'scribeseo' ) . '</p>';

			if ( wp_verify_nonce( $data['scribe-google-plus-nonce'], 'scribe-research-google-plus' ) )
				$results = self::build_gplus_comments( $data['scribe-keyword'] );
...
<script type="text/javascript" src="http://www.google.com/trends/embed.js?hl=en-US&q=___KEYWORD___&content=1&cid=RISING_QUERIES_0_0&export=5&w=300&h=420"></script>';
		...
		public static function ajax_handle_build_links() {
			...
			if(isset($data['scribe-build-links-nonce']) && wp_verify_nonce($data['scribe-build-links-nonce'], 'scribe-build-links')) {
				...
				$keywords = array_filter( $data['scribe-keywords'] );
				...
				if ( is_wp_error( $analysis ) ) {
					...
					switch( $type ) {
						case 'scr':
							...
							$link_analysis->score_description = sprintf( esc_js( __( scribe_get_link_building_score_description( $link_analysis->score ) ) ), implode( esc_js( __( ' and ', 'scribeseo' ) ), $keywords ) );
							break;
						case 'int':
		...
		public static function enqueue_administrative_resources($hook) {
			...
			wp_enqueue_style('scribe-bootstrap-necessary', plugins_url('resources/backend/bootstrap/bootstrap-necessary.css', __FILE__), array(), '1.4.0');
			wp_enqueue_script('scribe-bootstrap-twipsy', plugins_url('resources/backend/bootstrap/bootstrap-twipsy.js', __FILE__), array('jquery'), '1.4.0');
			wp_enqueue_script('scribe-bootstrap-popover', plugins_url('resources/backend/bootstrap/bootstrap-popover.js', __FILE__), array('jquery', 'scribe-bootstrap-twipsy'), '1.4.0');

			// jQuery Autocomplete - Prevent collision with WordPress SEO
			if(!wp_script_is('jquery-ui-autocomplete')) {
				wp_enqueue_script('jquery-ui-autocomplete', plugins_url('resources/backend/jquery-ui-autocomplete.min.js', __FILE__), array('jquery', 'jquery-ui-core'), self::VERSION, true);
			}
			wp_enqueue_script('jquery-ui-autocomplete-html', plugins_url('resources/backend/jquery-ui-autocomplete-html.js', __FILE__), array('jquery-ui-autocomplete'), self::VERSION, true);

			wp_enqueue_script('scribe-jqplot', plugins_url('resources/backend/jqplot/jquery.jqplot.min.js', __FILE__), array('jquery'), '1.0.0b2_r947');
			wp_enqueue_script('scribe-jqplot-point-labels', plugins_url('resources/backend/jqplot/plugins/jqplot.pointLabels.min.js', __FILE__), array('scribe-jqplot'), '1.0.0b2_r947');
			wp_enqueue_script('scribe-backend', plugins_url('resources/backend/scribe.js', __FILE__), array('jquery', 'thickbox', 'jquery-ui-tabs', 'scribe-jqplot', 'suggest'), self::VERSION);

			wp_enqueue_style('scribe-jqplot', plugins_url('resources/backend/jqplot/jquery.jqplot.min.css', __FILE__), array(), '1.0.0b2_r947');
			wp_enqueue_style('scribe-backend', plugins_url('resources/backend/scribe.css', __FILE__), array('thickbox'), self::VERSION);
		...
		public static function handle_versions() {
			if ( isset( $_GET['scribe-dismiss-premise-nag'] ) && 1 == $_GET['scribe-dismiss-premise-nag'] ) {
				update_option(self::SHOW_PREMISE_NAG_KEY, 'no');
				wp_redirect( add_query_arg( array( 'page' => urlencode( self::SETTINGS_SUB_PAGE_SLUG_SETTINGS ) ), admin_url( 'admin.php' ) ) );
				exit;
			}
		}
		...
		public static function process_compatibility_check() {
			...
			if(!empty($data['send-scribe-compatibility-report-nonce']) && wp_verify_nonce($data['send-scribe-compatibility-report-nonce'], 'send-scribe-compatibility-report')) {
				...
				$send_successful = self::send_test_results($compatibility['name'], $compatibility['email'], $compatibility['issue']);
		...
		public static function display_link_building_meta_box($post) {
			$content_analysis = self::get_content_analysis($post->ID);
			if ( ! empty( $content_analysis ) )
				$content_score = $content_analysis->docScore;
		...
		public static function display_keyword_iframe() {
			...
			switch($tab) {
				...
				default:
					...
					if($previous) {
						$keywords = self::get_archived_keyword_suggestions( $post_id, $user_id );
						$title = esc_html__('Previous Keyword Suggestions', 'scribeseo');
					} else {
						$queried_keyword = urldecode($data['scribe-keyword']);
						$keywords = self::get_keyword_suggestions( $queried_keyword, $post_id, $user_id );

						$title = sprintf( esc_html__( 'Keyword Suggestions for %1$s', 'scribeseo' ), '<em>' . esc_html( $queried_keyword ) . '</em>' );
					}
...
//			$social_activity = self::build_social_activity ( '', $link_building_info );
		...
		private static function get_account($api_key = null) {
			...
			if(empty($api_key)) {
				...
				$result = self::$scribe_api_account;
			} else {
				$scribe_api = new Scribe_API($api_key, 1 == $settings['security-method']);
				$result = $scribe_api->get_user_details();
			}

			return $result;
		}

		private static function get_content_analysis_evaluations_remaining( $account ) {

			return self::get_evaluations_remaining( $account, 'ContentAnalysis' );

		}

		private static function get_keyword_evaluations_remaining($account) {
		...
		private static function get_keyword_recommendations($possessed_keys) {
			...
			if(empty($suggestions)) {
				...
				if(!is_wp_error($suggestions)) {
					foreach($suggestions as $suggestion) {
						$suggestion->time_retrieved = current_time('timestamp');
					}
					self::update_archived_keyword_suggestions($suggestions, $post_id, $user_id);
		...
		private static function update_archived_keyword_suggestions( $keywords, $post_id, $user_id = null ) {
			global $wpdb;
			...
			$archived = array_filter(array_slice($archived, 0, 20));

			$base_key = self::PREVIOUS_KEYWORD_SUGGESTIONS_KEY . $post_id;

			update_user_option( $user_id, $base_key, $archived );
			wp_cache_set( $wpdb->prefix . $base_key, $keywords, $user_id, time() + self::CACHE_PERIOD );
		...
		private static function build_social_activity( $keyword, $links = null ) {
			...
			$twitter_format = '<li class="scribe-google-plus-item"><a href="%1$s" class="img" target="_blank"><img src="%2$s" /></a><h4><a href="%1$s" target="_blank">%1$s</a> <span class="scribe-google-plus-klout-score">%4$d</span></h4> %3$s</li>';
		...
		private static function send_test_results($name, $email, $issue) {
			$name = trim($name);
			$email = trim($email);
			$issue = trim($issue);
